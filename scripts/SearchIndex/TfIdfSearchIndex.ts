...
import getNestedFieldValue from "./Common";
...
export class TfIdfSearchIndex implements ISearchIndex {
    ...
    public search(tokens: Array<string>, corpus: Array<Object>): Array<Object> {
        ...
        return documents.sort(
            (documentA, documentB) =>
                calculateTfIdf(tokens, documentB, corpus) - calculateTfIdf(tokens, documentA, corpus)
        );
    }

    private _createCalculateIdf(): (token: string, documents: object[]) => number {
        ...
        return function calculateIdf(token: string, documents: Array<Object>): number {
            if (!tokenToIdfCache[token]) {
                var numDocumentsWithToken: number =
                    typeof tokenMap[token] !== "undefined" ? tokenMap[token].$numDocumentOccurrences : 0;
    ...
    private _createCalculateTfIdf(): (token: Array<string>, document: object, documents: Array<object>) => number {
        const tokenMap = this._tokenMap;
        ...
        return function calculateTfIdf(tokens: Array<string>, document: Object, documents: Array<Object>): number {
            ...
            for (var i = 0, numTokens = tokens.length; i < numTokens; ++i) {
                ...
                if (uidFieldName instanceof Array) {
                    ...
                    uid = document && (document as any)[uidFieldName];
                }

                var termFrequency: number =
                    typeof tokenMap[token] !== "undefined" && typeof tokenMap[token].$uidMap[uid] !== "undefined"
