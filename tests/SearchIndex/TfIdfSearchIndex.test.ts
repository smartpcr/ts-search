...
import { TfIdfSearchIndex } from "../../scripts/SearchIndex/TfIdfSearchIndex";

describe('Search', function() {
    var documents: Array<Object>, search: Search, uid: number;
  
    var addDocument = function(title: string): any {
      var document = {
        uid: ++uid,
        title: title
      };
    ...
    var calculateIdf = function(numDocumentsWithToken: number): number {
      return 1 + Math.log(search.documents.length/(1 + numDocumentsWithToken));
    };
  
    var assertIdf = function(term: string, numDocumentsWithToken: number): void {
      var _calculateIdf = (search.searchIndex as any)._createCalculateIdf();
      expect(_calculateIdf(term, search.documents)).toEqual(calculateIdf(numDocumentsWithToken));
    };
    ...
    var calculateTfIdf = function(numDocumentsWithToken: number, tokenCountInDocument: number): number {
      return calculateIdf(numDocumentsWithToken) * tokenCountInDocument;
    };
  
    var assertTfIdf = function(terms: Array<string>, document: Object, expectedTfIdf: number): void {
      var _calculateTfIdf = (search.searchIndex as any)._createCalculateTfIdf();
      expect(_calculateTfIdf(terms, document, search.documents)).toEqual(expectedTfIdf);
    };
